...
def make_character_index(text=text, characters=CHARACTERS):
    ...
    for char in characters:
        if isinstance(char, str):
            char_search = re.compile(char.lower())
            dic_key = char.lower()
        else:
            char_search = re.compile('|'.join(char).lower())
            dic_key = char[0].lower()
        for num, line in enumerate(data):
            if bool(char_search.search(line.lower())):
                result[dic_key].append(num)
    return result
