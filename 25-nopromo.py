...
class Promo:
    ...
    def __init__(self, bites_done=bites_done):
        self.bites_done = list(bites_done)
    ...
    def _pick_random_bite(self):
        if len(available_bytes) == 0:
            ...
            return random.choice(bites_left)

    def new_bite(self):
        bite = self._pick_random_bite()
        self.bites_done.append(bite)
        return bite
    
    def __str__(self):
        print(f'Bites Done: {self.available_bites}')
    
    def __repr__(self):
        print(f'{type(self.__name__}({self.bites_done})')